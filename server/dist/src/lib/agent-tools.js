...
exports.agentTools = exports.CORE_TOOL_NAMES = exports.ihsanToolPrompts = void 0;
...
const SKILLS_DIR = (0, node_path_1.join)(process.cwd(), "skills", "slides");
function loadSkillFile(filename) {
    try {
        return (0, node_fs_1.readFileSync)((0, node_path_1.join)(SKILLS_DIR, filename), "utf-8");
    }
    catch {
        return "";
    }
...
async function callIhsanTool(toolName, userPrompt) {
    const systemPrompt = exports.ihsanToolPrompts[toolName];
    if (!systemPrompt)
        throw new Error(`Unknown Ihsan tool: ${toolName}`);
    const model = (0, anthropic_1.anthropic)("claude-sonnet-4-5-20250929");
    const result = (0, ai_2.streamText)({
        model,
        system: systemPrompt,
...
function filterTools(allowedNames) {
    ...
    const filtered = {};
    for (const [key, value] of Object.entries(exports.agentTools)) {
        if (allowed.has(key)) {
            filtered[key] = value;
        }
    }
...
exports.agentTools = {
    ...
    web_search: (0, ai_1.tool)({
        description: "Search the web for information. Returns titles, snippets, and URLs. Use this to find current information, research topics, or discover relevant pages to fetch.",
        ...
        execute: async ({ query }) => {
            ...
            const res = await fetch(url, {
                headers: {
                    "User-Agent": "Mozilla/5.0 (compatible; IhsanAgent/1.0; +https://ihsan.ai)",
                },
            ...
            const results = [];
            const resultBlocks = html.match(/<div class="result[^"]*results_links[^"]*"[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/gi);
            if (resultBlocks) {
    ...
    web_fetch: (0, ai_1.tool)({
        description: "Fetch a web page and extract its text content. Use this to read articles, documentation, or any web page.",
        inputSchema: zod_1.z.object({
        ...
        execute: async ({ url }) => {
            ...
            if (contentType.includes("html")) {
                textContent = (0, html_to_text_1.htmlToText)(html);
            }
    ...
    // ── Code & Artifacts ───────────────────────────────────────────────
    run_javascript: (0, ai_1.tool)({
        description: "Execute JavaScript code and return the result. Useful for calculations, data processing, or generating structured output. The code runs in a sandbox with no access to require, process, or the file system. Use console.log() for output.",
        inputSchema: zod_1.z.object({
        ...
        execute: async ({ code }) => {
            ...
            try {
                ...
                const result = node_vm_1.default.runInContext(code, context, {
                    timeout: 5000,
                    displayErrors: true,
                });
                return {
    ...
    create_artifact: (0, ai_1.tool)({
        description: "Create a downloadable file/artifact for the user. Use this when the user asks you to write code, create documents, or generate any file they might want to download.",
        inputSchema: zod_1.z.object({
            title: zod_1.z.string().describe("A short title for the artifact (e.g. 'fibonacci.py')"),
            language: zod_1.z.string().describe("The programming language or file type (e.g. 'python', 'javascript', 'html', 'markdown')"),
            content: zod_1.z.string().describe("The full content of the file"),
        }),
    ...
    // ── Mermaid Diagram ────────────────────────────────────────────────
    create_diagram: (0, ai_1.tool)({
        description: "Create a Mermaid diagram (flowchart, sequence diagram, mind map, timeline, architecture diagram, etc.). The diagram will be rendered visually in the browser. Use this to visualize processes, architectures, comparisons, timelines, or any concept that benefits from a visual representation.",
        inputSchema: zod_1.z.object({
            title: zod_1.z.string().describe("A short title for the diagram"),
            diagram: zod_1.z.string().describe("The Mermaid diagram code (e.g. 'graph TD; A-->B; B-->C;')"),
        }),
    ...
    generate_slides: (0, ai_1.tool)({
        description: "Generate a full presentation/slide deck using Ihsan Slides. IMPORTANT: Before calling this tool, you should have already completed Steps 0-3 of the workflow (intake, research, outline, visual planning). Pass your research and outline as parameters so the inner model can build a rich, evidence-backed deck.",
        inputSchema: zod_1.z.object({
            topic: zod_1.z.string().describe("The presentation topic"),
            audience: zod_1.z.string().describe("Target audience (e.g., investors, staff, customers, students)").default("general"),
            goal: zod_1.z.enum(["persuade", "educate", "report", "pitch"]).describe("Presentation goal").default("educate"),
            duration_min: zod_1.z.number().describe("Presentation duration in minutes").default(10),
            tone: zod_1.z.enum(["executive", "technical", "friendly", "academic"]).describe("Presentation tone").default("friendly"),
            constraints: zod_1.z.string().describe("Optional constraints (e.g., 'no stock photos', 'cite sources')").optional(),
            research: zod_1.z.string().describe("Bullet-point claims with source URLs from your web research (Steps 0-1)").optional(),
            outline: zod_1.z.string().describe("Slide titles + one-sentence takeaways from your outline (Step 2)").optional(),
        }),
...
REQUIRED: Every slide must have an [Image: ...] tag with a specific, vivid image description.${researchSection}${outlineSection}`;
    ...
    generate_document: (0, ai_1.tool)({
        description: "Generate a professional document using Ihsan Docs. Use this for reports, proposals, essays, letters, memos, articles, guides, business plans, etc.",
        inputSchema: zod_1.z.object({
            brief: zod_1.z.string().describe("What document to create and any specific requirements"),
        }),
        execute: async ({ brief }) => {
    ...
    generate_code: (0, ai_1.tool)({
        description: "Generate complete, production-ready code using Ihsan Developer. Use this for full applications, components, scripts, or any substantial code the user needs. More powerful than run_javascript — this creates full files with imports, error handling, and setup instructions.",
        inputSchema: zod_1.z.object({
            spec: zod_1.z.string().describe("What code to generate — language, framework, and requirements"),
        }),
        execute: async ({ spec }) => {
    ...
    generate_design: (0, ai_1.tool)({
        description: "Generate a UI/UX design specification using Ihsan Designer. Includes color palette, typography, layout, components, CSS/HTML code, responsive notes, and accessibility guidelines.",
        inputSchema: zod_1.z.object({
            brief: zod_1.z.string().describe("What to design — app, website, component, logo, etc."),
        }),
        execute: async ({ brief }) => {
    ...
    generate_spreadsheet: (0, ai_1.tool)({
        description: "Generate data tables and analysis using Ihsan Sheets. Creates well-structured markdown tables with realistic data, formulas, and analysis.",
        inputSchema: zod_1.z.object({
            request: zod_1.z.string().describe("What data or analysis to generate"),
        }),
    ...
    generate_image_prompts: (0, ai_1.tool)({
        description: "Generate detailed AI image generation prompts using Ihsan Image. Creates 4 varied prompts optimized for DALL-E, Midjourney, and Stable Diffusion with technical parameters.",
        inputSchema: zod_1.z.object({
            description: zod_1.z.string().describe("What image the user wants to create"),
        }),
    ...
    generate_music: (0, ai_1.tool)({
        description: "Generate music composition, lyrics, and production specs using Ihsan Music. Includes chord progressions, lyrics with structure, tempo, instrumentation, and AI music prompts for Suno/Udio.",
        inputSchema: zod_1.z.object({
            request: zod_1.z.string().describe("What music/song to create — genre, mood, topic, etc."),
        }),
        execute: async ({ request }) => {
    ...
    generate_video_script: (0, ai_1.tool)({
        description: "Generate a complete video production plan using Ihsan Video. Includes script, shot list, storyboard descriptions, B-roll suggestions, and AI video prompts for Sora/Runway/Kling.",
        inputSchema: zod_1.z.object({
            concept: zod_1.z.string().describe("What video to create — topic, style, audience, duration"),
        }),
