...
const runCode: Skill = {
  ...
  description:
    "Execute code in a secure cloud sandbox. Supports Python, JavaScript, and Shell. " +
  ...
  parameters: z.object({
    ...
    code: z.string().describe("The code to execute in the cloud sandbox"),
  }),

  execute: async (params: { language: string; code: string }) => {
    ...
    try {
      ...
      throw new Error(
        "E2B Code Interpreter is not available. " +
    ...
    if (!apiKey) {
      throw new Error(
        "E2B_API_KEY is not set. Add it to your .env.local file. " +
