...
async function walkDir(
  ...
  let dirContents;
  try {
    dirContents = await readdir(dirPath, { withFileTypes: true });
  } catch {
...
const listFiles: Skill = {
  ...
  description:
    "List files and directories. Use this to explore the codebase, map the project " +
  ...
  parameters: z.object({
    dir_path: z
      ...
      .default(".")
      .describe(
        "Relative path from project root (e.g. '.', 'src', 'skills', 'src/agent-os')"
      ),
    recursive: z
      .boolean()
      .default(false)
  ...
  execute: async (params: {
    ...
    const normalized = normalize(dir_path).replace(/\\/g, "/");

    if (isAbsolute(normalized)) {
      throw new Error(
        "ACCESS DENIED: Absolute paths are not allowed. Use a relative path from the project root."
      );
    }

    if (normalized.startsWith("..")) {
      throw new Error(
        "ACCESS DENIED: Path traversal (..) is not allowed. Stay within the project."
      );
    }

    const targetDir = join(ROOT, normalized);

    // Verify the resolved path is still within the project
    const resolvedRelative = relative(ROOT, targetDir);
    if (resolvedRelative.startsWith("..")) {
      throw new Error(
        "ACCESS DENIED: The resolved path escapes the project root."
      );
    }
    ...
    let fileCount = 0;
    let dirCount = 0;
    function countEntries(entries: TreeEntry[]) {
      for (const e of entries) {
        if (e.type === "file") fileCount++;
        else dirCount++;
