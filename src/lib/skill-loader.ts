...
const SPEC_FILES = ["WORKFLOW.md", "QUALITY_GATES.md", "SLIDES_SPEC.md"] as const;

// ─── Load Skill Spec Files ──────────────────────────────────────────

function loadSkillSpecFiles(skillDir: string): Record<string, string> {
  const specs: Record<string, string> = {};
  const fullDir = path.join(process.cwd(), skillDir);
...
function extractSpecSummary(specContent: string): string {
  ...
  if (inputsMatch) sections.push(inputsMatch[0].trim());

  const slideCountMatch = specContent.match(/## Slide Count Rule[\s\S]*?(?=## Structure)/);
  if (slideCountMatch) sections.push(slideCountMatch[0].trim());

  const templatesMatch = specContent.match(/## Structure Templates[\s\S]*?(?=## Visual Rules)/);
  if (templatesMatch) sections.push(templatesMatch[0].trim());
 ...
 */
export function buildSkillsContext(enabledSkills: SkillForContext[]): string {
